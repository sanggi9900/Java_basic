...
// - 데이터를 연속적으로 저장
// - 순서 O (index 인덱스번호, 0부터 1씩 증가된는 값)
// - 중복 O
public class Array01 {
	public static void main(String[] args) {
		...
		// === 배열의 생성 =====
		// : 실제 데이터가 저장되는 것이 아니라 데이터 배열의 저장된 첫 번째 메모리 주소가 저장
		// >> new 데이터타입[배열의 길이];
		array = new int[3];
...
//      오류메세지 : Exception in thread "main" java.lang.ArrayIndexOutOfBoundsException: Index 3 out of bounds for length 3
//		at java_study/chapter03.A_Array.main(A_Array.java:52)
...
//		System.out.println(greet[5]);
		...
		int k =0; // 새로운 배열으 인데스 번호
		for(int i =0; i < originalLength; i++ ) {
			if(i == removeIndex) {
				continue; // 루프의 나마저 부분을 건너띄고 다음 반복을 진행
			}
			// k, i
			// 0, 0 = [1]
			// 1, 1 = [2]
